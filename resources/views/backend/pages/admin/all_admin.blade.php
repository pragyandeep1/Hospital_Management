					...
					<div class="table-responsive">
						<table id="tableDataExample" class="table">
							...
							<tbody>
								...
								<tr>
									<td>{{$key+1}}</td>
									<td><img src="{{(!empty($item->photo)) ? url('upload/admin_images/'.$item->photo) : url('upload/no-image.jpg')}}" style="width: 70px; height: 40px;"></td>
									<td>{{$item->name}}</td>
									<td>{{$item->email}}</td>
									<td>{{$item->phone}}</td>
									<td>
									...
									<td>
										...
										<a href="{{route('delete.admin',$item->id)}}" class="btn btn-inverse-danger" title="Delete">
